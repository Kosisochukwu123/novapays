...
export const executeTrade = async (req, res) => {
  try {
    const { symbol, name, assetType, action, quantity, priceAtTrade } =
      req.body;
...
export const resolveTrade = async (req, res) => {
  try {
    ...
    if (!["win", "loss"].includes(outcome)) {
      return res
        .status(400)
        .json({ message: "Outcome must be either win or loss" });
    }
    ...
    if (trade.status === "closed") {
      return res
        .status(400)
        .json({ message: "This trade has already been resolved" });
    }

    const parsedReturn = parseFloat(returnAmount) || 0;
    const profitLoss =
      outcome === "win" ? parsedReturn - trade.total : -trade.total;

    if (outcome === "win") {
      await User.findByIdAndUpdate(trade.user._id, {
    ...
    await createNotification(trade.user._id, {
      title: outcome === "win" ? "Trade Won!" : "Trade Closed",
      message:
        outcome === "win"
          ? `Your ${trade.symbol} trade won! $${parsedReturn} credited to your account`
          : `Your ${trade.symbol} trade has been closed`,
      type: "trade",
      link: "/trading",
    });
