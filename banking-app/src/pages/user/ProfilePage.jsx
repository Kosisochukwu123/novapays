...
const InputField = ({ label, type = 'text', name, value, onChange, icon: Icon, error, required }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ position: 'relative' }}>
        {Icon && (
          <Icon size={15} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: '#64748b' }} />
        )}
        <input
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ ...card, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{ width: 60, height: 60, borderRadius: 16, backgroundColor: '#38bdf8', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#0f172a', fontSize: 20, fontWeight: 700, flexShrink: 0 }}>
              {initials}
            </div>
            <div>
              <p style={{ color: '#fff', fontWeight: 600, fontSize: 18 }}>{user?.fullName}</p>
              <p style={{ color: '#64748b', fontSize: 14, marginTop: 2 }}>{user?.email}</p>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 12, padding: '2px 10px', borderRadius: 20, backgroundColor: 'rgba(56,189,248,0.1)', color: '#38bdf8', border: '1px solid rgba(56,189,248,0.2)' }}>
                  {user?.role || 'user'}
                </span>
                <span style={{ fontSize: 12, padding: '2px 10px', borderRadius: 20, backgroundColor: user?.status === 'active' ? 'rgba(34,197,94,0.1)' : 'rgba(245,158,11,0.1)', color: user?.status === 'active' ? '#4ade80' : '#fbbf24', border: `1px solid ${user?.status === 'active' ? 'rgba(34,197,94,0.2)' : 'rgba(245,158,11,0.2)'}` }}>
                  {user?.status || 'active'}
                </span>
              </div>
            </div>
          ...
          <div style={{ textAlign: 'right' }}>
            <p style={{ color: '#64748b', fontSize: 11, marginBottom: 4 }}>Account Balance</p>
        ...
        <div style={card}>
          <p style={{ color: '#fff', fontWeight: 600, fontSize: 16, marginBottom: 20 }}>
        ...
        <div style={card}>
          ...
          {langSaved && (
            <div style={{ marginBottom: 14, padding: '10px 14px', borderRadius: 10, backgroundColor: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.2)', color: '#4ade80', fontSize: 13, display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    backgroundColor: isActive ? 'rgba(56,189,248,0.1)' : 'rgba(255,255,255,0.03)',
                ...
                >
                  ...
                  <span style={{ color: isActive ? '#38bdf8' : '#f1f5f9', fontSize: 12, fontWeight: isActive ? 600 : 400 }}>
                    {lang.native}
                  </span>
                  <span style={{ color: '#64748b', fontSize: 10 }}>{lang.label}</span>
                  {isActive && (
            ...
            {pwForm.newPassword.length > 0 && (
              <div>
                <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
                  {[1,2,3,4].map(i => {
                    const strength = pwForm.newPassword.length >= 12 ? 4
                                   : pwForm.newPassword.length >= 8  ? 3
                                   : pwForm.newPassword.length >= 5  ? 2
                                   : 1;
                    const colors = ['#ef4444','#f59e0b','#38bdf8','#22c55e'];
                    return (
                      <div key={i} style={{ flex: 1, height: 4, borderRadius: 2, backgroundColor: i <= strength ? colors[strength - 1] : 'rgba(255,255,255,0.1)', transition: 'background-color 0.3s' }} />
                    );
                  })}
                </div>
                <p style={{ color: '#64748b', fontSize: 11 }}>
                  {pwForm.newPassword.length < 5  ? 'Too short'
                  : pwForm.newPassword.length < 8  ? 'Weak'
                  : pwForm.newPassword.length < 12 ? 'Good'
                  : 'Strong'}
                </p>
              </div>
            )}
            ...
            <div>
              <button
                ...
                style={{
                  ...
                  backgroundColor: pwLoading ? '#334155' : 'rgba(255,255,255,0.08)',
