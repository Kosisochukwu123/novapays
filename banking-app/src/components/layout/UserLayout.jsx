...
import {
  LayoutDashboard, Send, History, User, LogOut,
  Menu, X, ChevronRight, ArrowDownCircle,
  TrendingUp, Landmark, PiggyBank, ArrowDownToLine,
} from 'lucide-react';
...
export default function UserLayout({ children }) {
  ...
  const SidebarContent = () => (
    <div style={{
      ...
      {/* User badge */}
      <div style={{ margin: '14px 12px 8px', padding: '12px', borderRadius: 12, backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: '50%', backgroundColor: '#38bdf8', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#0f172a', fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
            {initials}
          </div>
          <div style={{ minWidth: 0 }}>
            ...
            <p style={{ color: '#64748b', fontSize: 11, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', margin: 0 }}>
              {user?.email}
            </p>
          </div>
        </div>
      ...
      {/* Nav items */}
      <nav style={{ flex: 1, padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: 4, overflowY: 'auto' }}>
      ...
      <div style={{ padding: '12px 10px 16px', borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div style={{ padding: '4px 8px' }}>
          <LanguageSwitcher />
        </div>
        <button
          onClick={handleLogout}
          style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 10, width: '100%', background: 'none', border: '1px solid transparent', color: '#94a3b8', fontSize: 14, fontWeight: 500, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif", transition: 'all 0.15s' }}
  ...
  return (
    <div style={{
        ...
        <header style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <button
              ...
              style={{ background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer', padding: 4, display: 'none', alignItems: 'center' }}
            >
              <Menu size={22} />
            </button>

            {/* Mobile: platform logo in topbar */}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            {/* Thin divider */}
            <div style={{ width: 1, height: 22, backgroundColor: 'rgba(255,255,255,0.08)' }} />
      ...
      {/* ── Chat widget — inside root so z-index stacks correctly ── */}
      <ChatWidget />
